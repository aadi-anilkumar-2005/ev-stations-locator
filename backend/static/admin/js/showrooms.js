function renderShowrooms() {
    const tbody = document.getElementById('showrooms-table-body');
    if(!tbody) return;
    
    tbody.innerHTML = data.showrooms.map(item => `
        <tr>
            <td class="px-4">
            ...
            <td class="px-4"><span class="badge rounded-pill bg-light text-dark border">${item.brand}</span></td>
            <td class="px-4">
    ...
    // Re-initialize icons for the newly created rows
    lucide.createIcons();
}
